...
from utils.visionLib.camera_utils import CapturingModule
from utils.visionLib import vision_utils as visu
...
class GUITemp:
    def __init__(self, master):
        ...
        self.roi_mask = cv2.imread("utils\\Images\\ROI_mask.png", 0)
        self.cell_id_mask = cv2.imread("utils\\Images\\cell_id_mask.png", 0)
        self.coefflist = np.load("utils\\visionLib\\coeff.npy")
    ...
    def robot_control_START_process(self):
        ...
        if self.output_type.get() == "":
            ...
            # save to database (local) here
            os.makedirs("storage", exist_ok=True)

            rowCount = self.rowCountDataTable.get() - 1
            rowdata = list(self.Input_table.item(
                str(rowCount), 'values'))
            print(rowdata)

            now = datetime.now()  # current date and time
            image_folder_name = os.path.join(
                    "storage", now.strftime("%m-%d-%Y__%H-%M-%S"))
            os.makedirs(image_folder_name, exist_ok=True)
            with open(os.path.join('storage', 'db.csv'), 'a') as the_file:
                the_file.write(
                    "{in_barcode:s},{out_barcode:s},{transfer_type:s},{output_transfer:s},{pick_position:s},{streak_position:s},{time_stamp:s},{colony_type:s},{colony_color:s},{colony_size:s},{colony_texture:s},{image_path:s}\n".format(
                        in_barcode=rowdata[0], out_barcode=rowdata[1], transfer_type=rowdata[2], output_transfer=rowdata[
                            3], pick_position=rowdata[4], streak_position=rowdata[5], time_stamp=rowdata[6],
                        colony_type=rowdata[7], colony_color=rowdata[8], colony_size=rowdata[
                            9], colony_texture=rowdata[10], image_path=image_folder_name
                    ))

            for colony in self.colony_patches:
                image, cell_id = colony
                cv2.imwrite(os.path.join(image_folder_name,
                            str(cell_id)+".png"), image)
                ...
                if self.runGetRef:
                    ...
                    for data in data_with_div:
                        ...
                        if j_div < 15:
                            ...
                            if not cell_id in self.detectedColony.keys():
                                ...
                                colony_rect = ((center, (radius*2, radius*2), 0))
                                colony_patch = visu.crop_rect(image, colony_rect)
                                self.colony_patches.append((colony_patch, cell_id))
                    ...
                    for i in range(8):
                        if not i in self.detectedColony.keys():
                            ...
                            output_coordinate_list.append((i, [self.detectedColony[i][2], [0.079, 0.907, 0.413, -0.006]]))
                    ...
                    # rowCount = self.rowCountDataTable.get() - 1
                    # rowdata = list(self.Input_table.item(str(rowCount), 'values'))
                    # print(rowdata)
...
class SiLA:
    ...
    def ringLightMotor(self, position):
        ...
        Return: None
        '''
        Robot_position = self.Robot_connect.set_dio(position, 'Ring_motor')
    ...
    def ringLightLamp(self,status):
        ...
        '''
        lamp_status = self.Robot_connect.set_dio(status, 'Ring_light')
        #print(value)
    ...
    def getTipCount(self):
        ...
        tip_count = abb_connector.global_tip_counter(self.Robot_connect, 0)
    ...
    def setTipCount(self,value = 96):
        ...
        Return: 96 if the reset is succesful.
                'Socket Error' - if unable to reset 

        '''
        if value <= 96:
            value = 96 - value
            status = abb_connector.global_tip_counter(self.Robot_connect, 1, 0)
    ...
    def callSubroutine(self,routine,*args):
        '''
        Input arguements: 1.Routine : a string which determines the subroutine called.
                                        'tip picking','colony picking','Straking1','Streaking2','DeepWell'

                          2.arg_1 and arg_2: arugements input for each subroutine.
                            Examples for each subroutine
                            1.routine: 'tip picking' 
                                arg_1: offsetx
                                arg_2: offsety

                            2.routine: 'colony picking'
                                arg_1: cell number
                                arg_2: Incoming or outgoing sequence

                            3.routine: Streaking1
                                arg_1: cell number
                                arg_2: type_selection ()
        
        Return: String - 'Done' : If succesful
                String - 'Error' : If fails
        
        
        '''
        status = self.Robot_connect.call_subroutine(routine,*args)
    ...
    def robot_control_START_process(self,output_type,streaking_type,transfer_type, co_ordinates):
        '''
        This function start the automatic streaking process after the colony has been selected.

        input arguements:
 
        1. output_location(string): 'Agar Plate' or 'Deep Well'

        2. streaking_type(string): 'Streaking Pattern 1' or 'Streaking Pattern 2'

        3.transfer_type(string) = '1 to 1' or '1 to Many'

        4.co-ordinates(list): co-ordinate values of the identified colonies

        Return:

        Data Values(list) : this list contains all the values of the cycle.
        ...
        self.global_tip_count = abb_connector.global_tip_counter(self.Robot_connect, 0) # get the tip_count from the Robot controller
        
        # get value based on selection
        if output_type == "Agar Plate":
            output_selected = streaking_type
            type_selected = transfer_type
            
        else:
            output_selected = output_type
            type_selected = transfer_type


        self.Robot_connect.set_dio(True, 'Ring_motor')
        self.Robot_connect.set_dio(False, 'Ring_light')
        time.sleep(0.5)
        
        new_value = abb_connector.Robot_start(self.Robot_connect, 
            self.global_tip_count, co_ordinates, output_selected, type_selected)

        #set the Global_tip counter after process
        abb_connector.global_tip_counter(self.Robot_connect, 1, self.global_tip_count + new_value)
    ...
    def colony_capture(self):
        '''
        This function is used to obtain detections of possible colonies
        input arguements - None

        Return:
                Image in Jpeg format, need to find pout how it can be displayed in SILA
        
        '''
        self.Robot_connect.set_dio(False, 'Ring_motor')
        time.sleep(0.5)
        self.Robot_connect.set_dio(True, 'Ring_light')
        time.sleep(3)
        
        new=SiLA_sub()
        new.processCVTask(False)
        
        colList = [["A"], ["B"]]
        return "capture_colony"

    def colony_identify(self,refVal):
        '''
        This function is used to obtain detections of possible colonies
        input arguements - 1. refVal(INT): integer number of the rerfernce colony selected.

        Return:
                1.Image in Jpeg format, need to find pout how it can be displayed in SILA.
                2.List of co-ordinates that needs to be passed to the robot via the 'robot_control_START_process' function

        '''

        #new=SiLA_sub()
        #new.txtReferenceID_func()
        #new.processCVTask(True)
...
class SiLA_sub:
    def __init__(self):
        ...
        #D:\\Projects\\SiLA 2\\agar-server\\agar-sila-server\\
        self.roi_mask = cv2.imread("utils\\Images\\ROI_mask.png", 0)
        self.cell_id_mask = cv2.imread("utils\\Images\\cell_id_mask.png", 0)
        self.coefflist = np.load("utils\\visionLib\\coeff.npy")
        self.sample_reference = None
    ...
    def txtReferenceID_func(self, refVal):
...
if __name__ == '__main__':
    ...
    # root.after(1000, GUI.initialize_connection)
    ...
    new = SiLA()
    # new.colony_capture()
    # new.ringLightMotor(True)
    # new.ringLightLamp(False)
